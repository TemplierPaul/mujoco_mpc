...
namespace mjpc {
std::string PLSim_Crane::XmlPath() const {
  return GetModelPath("plsim_crane/task.xml");
}
std::string PLSim_Crane::Name() const { return "PLSim_Crane"; }

void PLSim_Crane::ResidualFn::Residual(const mjModel* model, const mjData* data,
  ...
  mjtNum sigma_rad = plsim_payloadSwayAngle(tip_pos, payload_pos);
...
void PLSim_Crane::ResetLocked(const mjModel* model) {
...
// ---------------------------------------------
void PLSim_Crane::TransitionLocked(mjModel* model, mjData* data) {
    ...
    // === the following lines are commented to test motion prediction
    /*******
